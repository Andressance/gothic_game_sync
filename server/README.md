...
| `POST` | `/saves/{save_id}` | Subir un `.gss` como multipart `save` |
| `GET` | `/saves` | Listar partidas disponibles |
...
La API valida el identificador, la extensión y la cabecera `GSSPKG1` del paquete. El límite inicial es de 128 MiB por archivo. Para producción todavía faltan autenticación, TLS y almacenamiento de objetos.
